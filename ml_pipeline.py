...
def add_fvg_features(df):
    ...
    df['fvg_up'] = np.where(df['low'] > df['prev_high'], df['low'] - df['prev_high'], 0.0)
    df['fvg_down'] = np.where(df['high'] < df['prev_low'], df['prev_low'] - df['high'], 0.0)
    return df
...
def generate_signal():
    ...
    df = fetch_ohlc(symbol=os.getenv('SYMBOL', 'XAUUSD'),
    ...
    # 4) Use last model to predict latest bar
    latest_X = X.iloc[[-1]]
    model = models[-1]
